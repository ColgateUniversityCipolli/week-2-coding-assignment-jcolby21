...
\begin{enumerate}
  ...
  \textbf{Solution:}
<<size="scriptsize", warning=FALSE, message=FALSE>>=
trap.midpoints = a + 1:99*(delta.x) #all the points excluding the two endpoints a and b
trap.area = sum(0.5 * delta.x * (integrand(a) + 2 * sum(integrand(trap.midpoints)) + integrand(b))) #trapezoidal rule formula
@
...
riemann.sums <- function(fnct,                        # function to integrate
  ...
  if(method == "Left"){
    ...
    (delta.x <- (b-a)/n.rect)
    trap.midpoints = a + 1:(n.rect-1)*(delta.x) #all the points excluding the two endpoints a and b
    area = sum(0.5 * delta.x * (fnct(a) + 2 * sum(fnct(trap.midpoints)) + fnct(b))) #formula for trapezoidal rule
