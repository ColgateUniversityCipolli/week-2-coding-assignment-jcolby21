...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{trap.midpoints} \hlkwb{=} \hldef{a} \hlopt{+} \hlnum{1}\hlopt{:}\hlnum{99}\hlopt{*}\hldef{(delta.x)} \hlcom{#all the points excluding the two endpoints a and b}
\hldef{trap.area} \hlkwb{=} \hlkwd{sum}\hldef{(}\hlnum{0.5} \hlopt{*} \hldef{delta.x} \hlopt{*} \hldef{(}\hlkwd{integrand}\hldef{(a)} \hlopt{+} \hlnum{2} \hlopt{*} \hlkwd{sum}\hldef{(}\hlkwd{integrand}\hldef{(trap.midpoints))} \hlopt{+} \hlkwd{integrand}\hldef{(b)))} \hlcom{#trapezoidal rule formula}
\end{alltt}
\end{kframe}
\end{knitrout}
  \item Write a function that takes \texttt{a}, \texttt{b}, and number of 
  rectangles (\texttt{n.rect}) in as input and returns the Trapezoidal Rule 
...
\hldef{riemann.sums} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{fnct}\hldef{,}                        \hlcom{# function to integrate}
                         ...
                         \hlkwc{method} \hldef{=} \hlsng{"Trapezoidal"}\hldef{)\{}    \hlcom{# method to use (trap by default)}
  \hlcom{######################################}
  \hlcom{# Check Input}
  \hlcom{######################################}
  ...
  \hldef{\}}\hlkwa{else if}\hldef{(method} \hlopt{==} \hlsng{"Trapezoidal"}\hldef{)\{}
    \hldef{(delta.x} \hlkwb{<-} \hldef{(b}\hlopt{-}\hldef{a)}\hlopt{/}\hldef{n.rect)}
    \hldef{trap.midpoints} \hlkwb{=} \hldef{a} \hlopt{+} \hlnum{1}\hlopt{:}\hldef{(n.rect}\hlopt{-}\hlnum{1}\hldef{)}\hlopt{*}\hldef{(delta.x)} \hlcom{#all the points excluding the two endpoints a and b}
    \hldef{area} \hlkwb{=} \hlkwd{sum}\hldef{(}\hlnum{0.5} \hlopt{*} \hldef{delta.x} \hlopt{*} \hldef{(}\hlkwd{fnct}\hldef{(a)} \hlopt{+} \hlnum{2} \hlopt{*} \hlkwd{sum}\hldef{(}\hlkwd{fnct}\hldef{(trap.midpoints))} \hlopt{+} \hlkwd{fnct}\hldef{(b)))} \hlcom{#formula for trapezoidal rule}
...
\hlkwd{riemann.sums}\hldef{(}\hlkwc{fnct} \hldef{= integrand,}
             ...
             \hlkwc{method}\hldef{=}\hlsng{"Trapezoidal"}\hldef{)}
\end{alltt}
\begin{verbatim}
## [1] 8.6664
\end{verbatim}
