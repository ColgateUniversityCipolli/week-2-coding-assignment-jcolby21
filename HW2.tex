...
\hldef{trap.area} \hlkwb{=} \hlkwd{sum}\hldef{(}\hlnum{0.5}\hlopt{*}\hldef{(delta.x)}\hlopt{*}\hldef{(}\hlkwd{integrand}\hldef{(a)}\hlopt{+}\hlnum{2}\hlopt{*}\hldef{(}\hlkwd{integrand}\hldef{(trap.midpoints)}\hlopt{+}\hlkwd{integrand}\hldef{(b))))} \hlcom{#trapezoidal rule formula}
\hldef{trap.area}
\end{alltt}
\begin{verbatim}
## [1] 13.5564
